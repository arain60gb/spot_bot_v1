# bot.py  — updated to fix the fetch_data error
import sys
import os
import json
import csv
...
# Updated to use 1-hour timeframe analyzer
from technical_analyzer import QuickScalpAnalyzer, Colors, fetch_historical_data
...
class TradingStrategy:
    def __init__(self, symbol):
        # Use the 1-hour QuickScalpAnalyzer from new_technical_analyzer
        self.analyzer = QuickScalpAnalyzer(symbol, confirmation_candles=3, background_mode=True)
        self.analyzer.start()  # Start the analyzer
        self.symbol = symbol
        self.timeframe = "1h"  # Explicitly set to 1-hour timeframe
        
    def get_signals(self):
        if MODE in ["paper", "live"]:
            try:
                # Get the latest signal from the analyzer
                signal = self.analyzer.quick_decision()
                # Return in the format expected by the bot
                return {
                    self.timeframe: {
                        "overall_recommendation": signal.signal,
                        "debug": signal.debug
                    }
                }
            except Exception:
    ...
    def should_buy(self, signals):
        return signals.get(self.timeframe, {}).get("overall_recommendation", "") in ["Buy", "Strong Buy"]
    
    def should_sell(self, signals):
        return signals.get(self.timeframe, {}).get("overall_recommendation", "") in ["Sell", "Strong Sell"]
    
    def stop(self):
        """Stop the analyzer when the bot stops"""
...
class CryptoTradingBot:
    def __init__(self, symbol):
        ...
        self.timeframe = "1h"  # Set to 1-hour timeframe
        # position state
        ...
        # last analyzer snapshot for dashboard
        self.last_analyzer_snapshot = {}
        # live mode data fetcher & candle aggregator
        if MODE in ["paper", "live"]:
            try:
                # We'll use the analyzer's internal WebSocket for price data
                self.analyzer_ws = self.strategy.analyzer.ws
            except Exception:
                logger.exception("Could not get analyzer WebSocket")
                self.analyzer_ws = None
        ...
        print(f"Bot initialised for {symbol} – mode={MODE} – TF={self.timeframe}")
        logger.info("Bot init – %s – %s – %s", symbol, MODE, self.timeframe)
    ...
    def _signal_pill(self, rec: str):
        rec = rec or "Neutral"
        # Handle Strong Buy/Sell signals
        cmap = {
    ...
    def _build_analyzer_card(self):
        ...
        # Get analyzer debug info
        debug_info = snapshot.get('1h', {})
        consensus = snapshot.get('consensus', {})
        
        if debug_info:
            items += [
                html.P(f"HMA9: {debug_info.get('hma9', float('nan')):.4f} | HMA21: {debug_info.get('hma21', float('nan')):.4f}", className="mb-1"),
                html.P(f"MACD Hist: {debug_info.get('macd_hist', float('nan')):.4f}", className="mb-1"),
                html.P(f"RSI: {debug_info.get('rsi', float('nan')):.1f}", className="mb-1"),
                html.P(f"Stoch: {debug_info.get('stoch_k', float('nan')):.1f}/{debug_info.get('stoch_d', float('nan')):.1f}", className="mb-1"),
                html.P(f"Pattern: {debug_info.get('pattern', 'none')}", className="mb-1"),
                html.P(f"Quality: {debug_info.get('quality_score', 0):.0f}/100", className="mb-1"),
                html.P(f"Trend Strength: {debug_info.get('trend_strength', 0):.1f}", className="mb-1"),
                html.P(f"Volume Trend: {debug_info.get('volume_trend', 'unknown')}", className="mb-1"),
                html.P(f"Confirmation: {consensus.get('confirmation_count', 0)}/{consensus.get('confirmation_needed', 3)}", className="mb-1"),
            ]
        else:
            items.append(html.P("Analyzer data not available yet…", className="mb-1", style={"color": PALETTE["muted"]}))
        
        return dbc.Card(
            [
                dbc.CardHeader("1H Technical Analyzer"),
                dbc.CardBody(items),
            ],
            className="mt-2",
    ...
    def _build_position_card(self):
        ...
        items = [
            ...
            html.P(f"Timeframe: {self.timeframe}", className="mb-1"),
    ...
    def _setup_dashboard(self):
        ...
        app.layout = dbc.Container(
            [
                ...
                # Row 2: Analyzer (left) + Position (center) + Win/Loss (right)
                dbc.Row(
                    [
                        dbc.Col(html.Div(id="analyzer-card"), width=4),
        ...
        # NEW: one callback for Analyzer + Position + Pie
        @app.callback(
            [Output("analyzer-card", "children"),
             Output("position-card", "children"),
             Output("win-loss-pie", "figure")],
            Input("graph-update", "n_intervals"),
        )
        def _update_cards(_):
            return self._build_analyzer_card(), self._build_position_card(), self._build_winloss_pie()
    ...
    def _get_price(self):
        ...
        # Try to get price from the analyzer
        if hasattr(self, 'strategy') and hasattr(self.strategy, 'analyzer'):
            return self.strategy.analyzer.ws.current_price
    ...
    # In bot.py, update the _process_signal method in the CryptoTradingBot class
    ...
    def _process_signal(self, rec, price, ts):
        ...
        """
        # Extract the actual signal value from the dictionary if needed
        if isinstance(rec, dict):
            # rec is a dictionary from get_signals()
            signal_value = rec.get(self.timeframe, {}).get("overall_recommendation", "Neutral")
        else:
            # rec is already a string value
            signal_value = rec if rec else "Neutral"
        
        self.current_signal = signal_value
        self.last_price = price
        print(f"{Colors.BOLD}[{ts}] SIGNAL: {signal_value}{Colors.END}")
        
        if not self.trading_enabled:
            print(f"{Colors.YELLOW}[{ts}] Trading is disabled. Ignoring signal: {signal_value}{Colors.END}")
            return
        
        # When Flat: open positions on Buy/SB or Sell/SS
        if self.position_state == "Flat":
            if signal_value in ("Buy", "Strong Buy"):
                self._execute_buy(price, ts)  # OPEN LONG
                self.open_signal = signal_value
                self.seen_strong_follow_through = False
            elif signal_value in ("Sell", "Strong Sell"):
        ...
        if self.position_state == "Long":
            if signal_value in ("Sell", "Strong Sell"):
                self._execute_sell(price, ts, closed_by=f"signal:{signal_value}")  # CLOSE LONG
                self.open_signal = None
                self.seen_strong_follow_through = False
            return
        ...
        if self.position_state == "Short":
            if signal_value in ("Buy", "Strong Buy"):
                self._execute_buy(price, ts, closed_by=f"signal:{signal_value}")   # CLOSE SHORT
                self.open_signal = None
                self.seen_strong_follow_through = False
            return
    ...
    def _prime_live_candles(self, limit: int = 200):
        try:
            # Use the fetch_historical_data function from new_technical_analyzer
            # Format the symbol for the fetch function (remove slash)
            symbol_for_fetch = self.symbol.replace('/', '')
            candles = fetch_historical_data(symbol_for_fetch, "1h", limit)
            ...
            if candles:
                ...
                print(f"Successfully primed {len(self.candle_history)} 1h candles")
            else:
                print("No historical candles returned")
        except Exception as e:
            logger.exception("Prime live candles failed: %s", str(e))
    
    def _update_live_candle(self, now: datetime, price: float):
        secs = self._tf_seconds(self.timeframe)  # Use 1-hour timeframe
    ...
    def run_backtest(self, timeframe=None, days=30):
        tf = timeframe or self.timeframe  # Use 1-hour timeframe
        ...
        # For 1-hour backtest, we need to adjust the warmup period
        warmup = {"1h": 50}.get(tf, 100)  # Reduced warmup for 1h
        ...
        for i in range(start_idx, len(self.historical_data)):
            ...
            # Get signal from the analyzer (simplified for backtest)
            try:
                # Create a temporary analyzer for backtesting
                temp_analyzer = QuickScalpAnalyzer(self.symbol, background_mode=True)
                
                # Feed historical data to the analyzer
                for j in range(max(0, i-100), i+1):
                    hist_candle = self.historical_data.iloc[j]
                    temp_analyzer.ws.data['1h'].append({
                        'timestamp': hist_candle.name,
                        'open': float(hist_candle['open']),
                        'high': float(hist_candle['high']),
                        'low': float(hist_candle['low']),
                        'close': float(hist_candle['close']),
                        'volume': float(hist_candle.get('volume', 0)),
                        'is_closed': True
                    })
                
                # Get signal
                signal = temp_analyzer.quick_decision()
                rec = signal.signal
    ...
    # ── Live / paper loop -----------------------------------------------------
    # In bot.py, update the run_live_paper method in the CryptoTradingBot class

    def run_live_paper(self):
        print(f"{Colors.CYAN}Live/Paper mode – TF={self.timeframe}{Colors.END}")
        self._start_dash_server()
        ...
        while True:
            ...
            # Update analyzer snapshot for dashboard
            try:
                self.last_analyzer_snapshot = self.strategy.analyzer.last_snapshot
                
                # Debug: Get the raw signal from analyzer
                raw_signal = self.strategy.analyzer.quick_decision()
                print(f"{Colors.BLUE}[DEBUG] Analyzer raw signal: {raw_signal.signal}{Colors.END}")
                
                # Debug: Get the signals dictionary from strategy
                signals_dict = self.strategy.get_signals()
                print(f"{Colors.MAGENTA}[DEBUG] Signals dict: {signals_dict}{Colors.END}")
                
                # Debug: Extract the signal value
                extracted_signal = signals_dict.get(self.timeframe, {}).get("overall_recommendation", "Neutral")
                print(f"{Colors.YELLOW}[DEBUG] Extracted signal: {extracted_signal}{Colors.END}")
                
            except Exception as e:
                logger.exception("Error getting analyzer snapshot: %s", str(e))
            ...
            rec = signals.get(self.timeframe, {}).get("overall_recommendation", "")
            
            self._process_signal(signals, price, now)
            
            time.sleep(2)  # Check every 2 seconds for 1-hour timeframe
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Crypto Trading Bot (1-Hour Timeframe)")
    parser.add_argument("--timeframe", default="1h", choices=["1h"], help="Timeframe (only 1h supported)")
    ...
    # Only allow 1h timeframe
    if args.timeframe != "1h":
        print("Only 1-hour timeframe is supported. Setting to 1h.")
        args.timeframe = "1h"
    
    bot = CryptoTradingBot(symbol=args.symbol)
