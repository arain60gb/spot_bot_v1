# bot.py — updated to integrate with technical_analyzer.py
import sys
import os
import json
import csv
...
from technical_analyzer import (
    TechnicalAnalyzer,
    DataFetcher as AnalyzerDataFetcher,
    compute_trade_plan_1h_5m,
)
...
class TradingStrategy:
    def __init__(self, symbol, primary_tf="15m"):
        ...
        self.sentiment_score = 0.71  # Can be updated dynamically

    def get_signals(self):
        """Fetch signals from TechnicalAnalyzer."""
        try:
            results = self.analyzer.analyze_all_timeframes()
            signals = {}
            for tf, data in results.items():
                if data is not None:
                    rec = data.get("overall_recommendation", "Neutral")
                    signals[tf] = {"overall_recommendation": rec}
            return signals
        except Exception as e:
            logging.exception("Error fetching signals from TechnicalAnalyzer: %s", e)
            return {}

    def get_trade_plan(self):
        """Recompute trade plan using latest 1h and 5m data."""
        try:
            df_1h = self.analyzer.data_fetcher.fetch_data('1h', limit=200)
            df_5m = self.analyzer.data_fetcher.fetch_data('5m', limit=300)
            if df_1h is None or df_5m is None or df_1h.empty or df_5m.empty:
                return None
            return compute_trade_plan_1h_5m(df_1h, df_5m, sentiment=self.sentiment_score)
        except Exception as e:
            logging.exception("Failed to compute trade plan: %s", e)
            return None

    def should_buy(self, signal):
        return signal in ("Buy", "Strong Buy")

    def should_sell(self, signal):
        return signal in ("Sell", "Strong Sell")


# ── Main Bot ──────────────────────────────────────────────────────────────────
class CryptoTradingBot:
    def __init__(self, symbol=SYMBOL, primary_tf="15m"):
        self.symbol = symbol
        ...
        self.simulated_balance = {
            self.quote_asset: INITIAL_USDT,
            self.base_asset: INITIAL_TAO
        }
        ...
        # Setup exchange if live
        if MODE == "live":
            if not API_KEY or not API_SECRET:
                raise EnvironmentError("API keys required for live mode.")
    ...
    def _setup_logging(self):
        ...
        logging.addLevelName(logging.ERROR, f"{Colors.RED}ERROR{Colors.END}")
        logging.addLevelName(logging.INFO, f"{Colors.CYAN}INFO{Colors.END}")
    ...
    def _start_dash_server(self):
        ...
        app.layout = html.Div([
            dbc.Container([
                html.H2(f"{self.symbol} Trading Dashboard", className="text-center text-light mb-4"),
                dbc.Row([
                ...
                dbc.Row([
                    ...
                    dbc.Col(dcc.Graph(id="win-loss-pie"), width=4),
                ], className="mt-3"),
                html.Div(id="trades-table-wrap", className="mt-3"),
                dbc.Row([
                    dbc.Col([
                        dbc.Button("Start Trading", id="start-button", color="success", className="me-2"),
                        dbc.Button("Close Position", id="close-button", color="danger", className="me-2"),
                        dbc.Button("Force Long", id="longtrig-button", color="info", className="me-2"),
                        dbc.Button("Force Short", id="shorttrig-button", color="warning"),
                ...
                dbc.Row(dbc.Col(
                    html.Div("Close the browser or press Ctrl-C to stop the bot.",
        ...
        def _update_table_and_handle_buttons(_, close_clicks, start_clicks, shorttrig_clicks, longtrig_clicks):
            ...
            if trig == "close-button" and close_clicks and price:
                self.close_position(ts, price)
            elif trig == "start-button" and start_clicks:
                self.enable_trading()
            elif trig == "shorttrig-button" and shorttrig_clicks and price:
                self._execute_sell(price, ts, closed_by="Manual Short")
            elif trig == "longtrig-button" and longtrig_clicks and price:
                self._execute_buy(price, ts)

            return self._build_trades_table()

        self._dash_server = app
        self._dash_thread = threading.Thread(target=app.run_server, kwargs={'host': '127.0.0.1', 'port': 8050})
        self._dash_thread.daemon = True
        self._dash_thread.start()

    def _build_price_figure(self):
        with self._dash_lock:
            df = pd.DataFrame(self.candle_history[-200:])
            if df.empty:
        ...
        port_val = (self.simulated_balance[self.quote_asset] +
                    self.simulated_balance[self.base_asset] * (self.last_price or 1))

        pnl = port_val - (INITIAL_USDT + INITIAL_TAO * (self.candle_history[0]["open"] if self.candle_history else 1))
        pnl_color = PALETTE["trade_win"] if pnl >= 0 else PALETTE["trade_loss"]
        ...
        fig.update_layout(
            title=dict(
                text=(f"Total PnL: {pnl:,.2f} {self.quote_asset} • "
                      f"Signal: {self.current_signal} • "
                      f"Current: {self.last_price:,.4f} {self.quote_asset}<br>"
                      f"Trading: {'<span style=\"color:#15803d\">ENABLED</span>' if self.trading_enabled else '<span style=\"color:#b91c1c\">DISABLED</span>'}"),
                x=0.5,
    ...
    def _build_plan_card(self):
        ...
        return [
            html.H5("Trade Plan", className="card-title text-center text-light"),
            html.P(f"Trend: Long={plan.get('trend_long', False)} | Short={plan.get('trend_short', False)}", className="mb-1"),
            html.P(f"LongTrig: {plan.get('long_trigger', 0):.4f}", className="mb-1"),
            html.P(f"ShortTrig: {plan.get('short_trigger', 0):.4f}", className="mb-1"),
            html.P(f"SL Mult: {plan.get('sl_mult', 1.2):.2f} | TP Mult: {plan.get('tp_mult', 2.0):.2f}", className="mb-1"),
        ]

    def _build_position_card(self):
        if self.position_state == "Flat":
            return [html.H5("Position", className="card-title text-center text-light"), html.P("Flat", className="text-center")]
        pnl = (self.last_price - self.entry_price) * self.amount_base if self.position_state == "Long" else \
              (self.entry_price - self.last_price) * self.amount_base
        return [
            html.H5("Position", className="card-title text-center text-light"),
            html.P(f"{self.position_state} @ {self.entry_price:.4f}", className="mb-1"),
            html.P(f"Amount: {self.amount_base:.6f} {self.base_asset}", className="mb-1"),
            html.P(f"SL: {self.sl_level:.4f} | TP: {self.tp_level:.4f}", className="mb-1"),
            html.P(f"PnL: {pnl:.4f} {self.quote_asset}", className="mb-1", style={"color": "green" if pnl >= 0 else "red"}),
        ]

    def _build_winloss_pie(self):
        win = sum(1 for t in self.completed_trades if t.get("pnl", 0) > 0)
        loss = sum(1 for t in self.completed_trades if t.get("pnl", 0) < 0)
        total = win + loss
        if total == 0:
            win, loss = 1, 1
        fig = go.Figure(data=[go.Pie(labels=["Win", "Loss"], values=[win, loss], hole=0.55,
    ...
    def _build_trades_table(self):
        ...
        for trade in reversed(self.completed_trades[-10:]):
            profit_style = {"color": "green"} if trade.get("pnl", 0) >= 0 else {"color": "red"}
            rows.append(html.Tr([
                html.Td(trade.get("exit_time", "")[:19]),
                html.Td(trade.get("open_signal", "")),
                html.Td(f"{trade.get('entry_price', 0):.4f} → {trade.get('exit_price', 0):.4f}"),
                html.Td(f"{trade.get('pnl', 0):+.4f}", style=profit_style),
            ]))
        return [html.H5("Recent Trades", className="text-center text-light"), html.Table([
            html.Thead(html.Tr([html.Th("Time"), html.Th("Signal"), html.Th("Price"), html.Th("PnL")]),
                       style={"color": PALETTE["text"]}),
            html.Tbody(rows)
        ], className="table table-dark table-sm")]

    def _tf_seconds(self, tf):
        m = {'m': 60, 'h': 3600, 'd': 86400, 'w': 604800}
        return int(tf[:-1]) * m[tf[-1]]

    def _floor_time(self, dt, secs):
        return dt - timedelta(seconds=dt.second % secs, microseconds=dt.microsecond)
    ...
    def enable_trading(self):
        ...
        logging.info("Trading enabled via dashboard")

    def close_position(self, ts, price):
        if self.position_state == "Long":
            self._execute_sell(price, ts, closed_by="Manual")
        elif self.position_state == "Short":
            self._execute_buy(price, ts, closed_by="Manual")
        logging.info("Position closed manually")

    def _execute_buy(self, price, ts, closed_by=None):
        amount = FIXED_ORDER_SIZE_USDT / price
        fee = amount * 0.001
        amount -= fee
        if MODE == "live":
            print("Live buy not implemented.")
            return
        self._update_balance(self.quote_asset, -FIXED_ORDER_SIZE_USDT)
        ...
        self.open_signal = closed_by or "Buy"
        print(f"{Colors.GREEN}[{ts}] BUY @ {price:.4f} – {amount:.6f} {self.base_asset}{Colors.END}")
        print(f" Entry: {price:.4f}| SL: {self.sl_level:.4f}| TP: {self.tp_level:.4f}")
        self._log_trade("BUY_OPEN_LONG", ts, price, amount, fee)

    def _execute_sell(self, price, ts, closed_by=None):
        if self.position_state == "Long":
            amount = self.amount_base
            fee = amount * 0.001
            received = (amount - fee) * price
            self._update_balance(self.quote_asset, received)
            pnl = received - (self.entry_price * self.amount_base)
            self.completed_trades.append({
                "exit_time": ts, "exit_price": price, "amount": amount, "pnl": pnl,
                "open_signal": self.open_signal, "close_reason": closed_by or "Signal"
            })
            print(f"{Colors.RED}[{ts}] SELL CLOSE @ {price:.4f} – {amount:.6f} {self.base_asset}{Colors.END}")
            print(f" PnL: {pnl:+.4f} {self.quote_asset}")
            self._log_trade("SELL_CLOSE_LONG", ts, price, amount, fee)
        elif self.position_state == "Short":
    ...
    def _check_sl_tp_on_tick(self, ts, price):
        if self.position_state == "Long":
            if price <= self.sl_level:
                self._execute_sell(price, ts, closed_by="SL")
            elif price >= self.tp_level:
                self._execute_sell(price, ts, closed_by="TP")
    ...
    def _process_signal(self, rec, price, ts):
        ...
        if not self.trading_enabled:
            return
        if self.position_state == "Flat":
            if self.strategy.should_buy(rec):
                self._execute_buy(price, ts)
        elif self.position_state == "Long":
            if self.strategy.should_sell(rec):
                self._execute_sell(price, ts, closed_by="Signal")
    ...
    def _get_price(self):
        try:
            ...
            logging.exception("Price fetch error")
    ...
    def _log_trade(self, typ, ts, price, amount, fee):
        details = {
            "type": typ,
            ...
            "price": price,
            "amount_base": amount,
            "fee_quote": fee * price if typ.startswith("BUY") else fee * price,
        }
        if "CLOSE" in typ:
            match_key = f"BUY_OPEN_LONG_{amount}_{ts}"
            if match_key in self._open_trades_csv:
                open_d = self._open_trades_csv.pop(match_key)
                pnl = (price - open_d["price"]) * amount
                details["pnl"] = pnl
        self._log_to_csv(details)

    def _log_to_csv(self, details):
        csv_fn = Path(f"trade_cycle_{self.symbol.replace('/', '_')}_{MODE}.csv")
        csv_fn.parent.mkdir(parents=True, exist_ok=True)
        file_exists = csv_fn.exists()
        with open(csv_fn, 'a', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=list(details.keys()))
            if not file_exists:
                writer.writeheader()
    ...
    def _shutdown_dash_server(self):
        if self._dash_server:
            ...
            # No clean shutdown for Dash, but we can log
            self._dash_server = None
    ...
    def run_backtest(self, timeframe=None, days=30):
        ...
        self.backtest_fetcher = BacktestDataFetcher(self.symbol)
        ...
        if len(self.historical_data) <= warmup:
            print("Not enough candles for warm-up.")
            return
        start_idx = warmup
        ...
        for i in range(start_idx, len(self.historical_data)):
            ...
            ts = candle.name
            self._update_live_candle(ts, candle["close"])
            port_val = (self.simulated_balance[self.quote_asset] +
                        self.simulated_balance[self.base_asset] * candle["close"])
            dd = (peak_val - port_val) / peak_val * 100
            max_dd = max(max_dd, dd)
            if i % 50 == 0:
                self._process_signal("Neutral", candle["close"], ts)  # Placeholder
        ...
        print(f"{Colors.CYAN}Back-test completed – Net PnL: {net:.2f} ({net_pct:.2f}%) – Max DD: {max_dd:.2f}%{Colors.END}")
    ...
    def run_live_paper(self):
        ...
        signal_cooldown = timedelta(seconds=30)
        while self.running:
            ...
            try:
                ...
                self._check_sl_tp_on_tick(now, price)
                if last_signal_time is None or (now - last_signal_time) >= signal_cooldown:
                    signals = self.strategy.get_signals()
                    rec = signals.get(self.primary_tf, {}).get("overall_recommendation", "Neutral")
                ...
                logging.exception("Live loop error: %s", e)
                time.sleep(5)

    def _prime_live_candles(self, limit=200):
        try:
            df = self.live_fetcher.fetch_data(self.primary_tf, limit=limit)
            if df is not None and not df.empty:
                with self._dash_lock:
                    self.candle_history = [
                        {"timestamp": ts, "open": r["open"], "high": r["high"], "low": r["low"], "close": r["close"]}
                        for ts, r in df.iterrows()
                    ]
        except Exception:
            logging.exception("Prime live candles failed")

    def run(self, backtest_tf=None, backtest_days=30):
        if MODE == "backtest":
            return self.run_backtest(timeframe=backtest_tf, days=backtest_days)
        self.run_live_paper()


# ── Main ──────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    bot = CryptoTradingBot(SYMBOL, primary_tf="15m")
    bot.run(backtest_tf="15m", backtest_days=30)
