...
class BinanceWebSocket:
    ...
    async def _handle_message(self, data):
        ...
        if self.message_count % 5 == 0:  # Print every 5th message for debugging
            print(f"{Colors.BLUE}Received message #{self.message_count}{Colors.END}")
        ...
        if 'stream' in data and 'data' in data:
            ...
            if len(timeframe_parts) < 2:
                print(f"{Colors.RED}Invalid stream format: {stream}{Colors.END}")
                return
            ...
            if self.message_count <= 3:  # Print first 3 candles for debugging
                print(f"{Colors.MAGENTA}New candle: {timeframe} - Close: {candle['close']} - Closed: {candle['is_closed']}{Colors.END}")
        else:
    ...
    def _update_data(self, timeframe, candle):
        ...
        if candle['is_closed']:
            ...
            print(f"{Colors.GREEN}Added closed {timeframe} candle. Total: {len(self.data[timeframe])}{Colors.END}")
        else:
            ...
            if self.data[timeframe] and not self.data[timeframe][-1]['is_closed']:
                ...
                self.data[timeframe].append(candle)
                print(f"{Colors.YELLOW}Added new open {timeframe} candle. Total: {len(self.data[timeframe])}{Colors.END}")
                
    def get_data(self, timeframe):
        """Get the latest data for a specific timeframe as a DataFrame"""
        if not self.data[timeframe]:
            print(f"{Colors.YELLOW}No data for {timeframe} timeframe{Colors.END}")
            return None
...
def fetch_historical_data(symbol='TAOUSDT', interval='5m', limit=200):
    ...
    try:
        ...
        if not data:
            print(f"{Colors.RED}No historical data received for {symbol} {interval}{Colors.END}")
            return []
        ...
        print(f"{Colors.GREEN}Fetched {len(candles)} historical candles for {symbol} {interval}{Colors.END}")
        return candles
...
class QuickScalpAnalyzer:
    ...
    """
    def __init__(self, symbol='TAOUSDT', confirmation_candles=3):
        self.symbol = symbol
        ...
        # Signal confirmation mechanism - increased to 3 candles (15 minutes)
        self.confirmation_candles = confirmation_candles  # Number of candles to confirm signal
        self.current_signal = 'Neutral'  # Current unconfirmed signal
        self.confirmation_count = 0  # Count of consecutive candles with same signal
        self.confirmed_signal = 'Neutral'  # Last confirmed signal
        self.signal_history = deque(maxlen=10)  # Track recent signals for analysis
        
        # Signal quality scoring - increased threshold
        self.signal_quality_threshold = 75  # Minimum quality score (0-100)
        
        # Additional filters
        self.min_trend_strength = 20.0  # Minimum trend strength for signals
        self.min_volume_multiplier = 1.3  # Volume must be 1.3x average for confirmation
    ...
    def _volume_trend(self, volume: pd.Series, window: int = 5) -> str:
        ...
        if volume.iloc[-1] > vol_sma.iloc[-1] * 1.5:  # 50% above average
            return 'very_high'
        elif volume.iloc[-1] > vol_sma.iloc[-1] * 1.3:  # 30% above average
            return 'high'
        elif volume.iloc[-1] > vol_sma.iloc[-1]:  # Above average
            return 'above_avg'
        elif volume.iloc[-1] < vol_sma.iloc[-1] * 0.7:  # 30% below average
            return 'low'
        else:  # Below average
            return 'below_avg'
    ...
    def _one_tf_signal(self, df: pd.DataFrame) -> QuickSig:
        ...
        trend_threshold = 15 + volatility * 2  # Adjust based on market volatility
        ...
        # Trend strength filter - higher threshold
        ...
        # Buy signal conditions (multi-condition confirmation)
        ...
        # Sell signal conditions (multi-condition confirmation)
        ...
        min_conditions = 5  # Require at least 5 conditions for a signal
...
#       - Trend strength filtering
#       - Multi-condition signal generation
#       - Dynamic confirmation mechanism
#     """
#     def __init__(self, symbol='TAOUSDT', confirmation_candles=2):
#         self.symbol = symbol
#         self.ws = BinanceWebSocket(symbol)
#         self.last_snapshot = {
#             '5m': {},
...
#         self.signal_quality_threshold = 70  # Minimum quality score (0-100)
...
#         """Determine volume trend"""
#         vol_sma = self._volume_sma(volume, window)
#         if volume.iloc[-1] > vol_sma.iloc[-1] * 1.2:  # 20% above average
#             return 'high'
#         elif volume.iloc[-1] > vol_sma.iloc[-1]:  # Above average
#             return 'above_avg'
#         elif volume.iloc[-1] < vol_sma.iloc[-1] * 0.8:  # 20% below average
#             return 'low'
#         else:  # Below average
#             return 'below_avg'
...
#     def _calculate_signal_quality(self, signal_type, conditions_met, trend_strength, volume_trend) -> int:
#         """Calculate signal quality score (0-100)"""
#         # Base score from conditions met
#         base_score = len(conditions_met) * 20  # Each condition is worth 20 points
        
#         # Trend strength bonus (0-20 points)
#         trend_bonus = min(20, trend_strength)
        
#         # Volume trend bonus (0-20 points)
#         if volume_trend == 'high':
#             volume_bonus = 20
#         elif volume_trend == 'above_avg':
#             volume_bonus = 15
#         elif volume_trend == 'below_avg':
#             volume_bonus = 5
#         else:  # low
#             volume_bonus = 0
...
#         # Calculate total quality score
#         quality_score = min(100, base_score + trend_bonus + volume_bonus + pattern_bonus)
...
#         # Volume confirmation
#         volume_confirms_bull = volume_trend in ['high', 'above_avg']
#         volume_confirms_bear = volume_trend in ['high', 'above_avg']
...
#         # Determine signal based on conditions met
#         min_conditions = 4  # Require at least 4 conditions for a signal
...
#         quality_score = 0
#         if signal != 'Neutral':
#             quality_score = self._calculate_signal_quality(signal, conditions_met, trend_strength, volume_trend)
...
#             if new_signal != self.confirmed_signal:
#                 self.confirmed_signal = new_signal
#                 print(f"{Colors.CYAN}Signal confirmed: {new_signal} after {self.confirmation_count} candles{Colors.END}")
#         elif self.confirmation_count == 1:
#             print(f"{Colors.YELLOW}New signal detected: {new_signal} (confirmation: 1/{self.confirmation_candles}){Colors.END}")
#         else:
#             print(f"{Colors.BLUE}Signal continuing: {new_signal} (confirmation: {self.confirmation_count}/{self.confirmation_candles}){Colors.END}")
...
#         current_signal = consensus.get('current_signal', 'Neutral')
#         confirmation_count = consensus.get('confirmation_count', 0)
#         confirmation_needed = consensus.get('confirmation_needed', 2)
#         signal_5m = consensus.get('5m_signal', 'Neutral')
...
#         print(f"{Colors.BOLD}{Colors.BLUE}{'='*60}{Colors.END}")
#         print(f"{Colors.BOLD}Volatility: {Colors.YELLOW}{debug5.get('volatility', 0):.2f}%{Colors.END}")
#         print(f"{Colors.BOLD}Trend Strength: {Colors.YELLOW}{trend_strength:.1f} (Threshold: {debug5.get('trend_threshold', 0):.1f}){Colors.END}")
#         print(f"{Colors.BOLD}Volume Trend: {Colors.YELLOW}{volume_trend}{Colors.END}")
#         print(f"{Colors.BOLD}ATR: {Colors.YELLOW}{debug5.get('atr', 0):.2f}{Colors.END}")
...
#             print(f"{Colors.GREEN}BUY SIGNAL CONFIRMED:{Colors.END}")
#             print("- Multiple bullish conditions confirmed across indicators")
#             print("- Sufficient volume supports the upward movement")
#             print("- Trend strength is adequate for a reliable signal")
#             print(f"- Signal quality score: {quality_score}/100")
#             print("- Consider entering long position with confidence")
...
#             print(f"{Colors.RED}SELL SIGNAL CONFIRMED:{Colors.END}")
#             print("- Multiple bearish conditions confirmed across indicators")
#             print("- Sufficient volume supports the downward movement")
#             print("- Trend strength is adequate for a reliable signal")
#             print(f"- Signal quality score: {quality_score}/100")
#             print("- Consider entering short position with confidence")
...
#                 print(f"- {current_signal.upper()} signal detected but not yet confirmed")
#                 print(f"- Awaiting {confirmation_needed - confirmation_count} more candle(s) for confirmation")
#                 print("- Current signal may change before confirmation")
#                 print("- Wait for confirmed signal before trading")
...
# # Standalone execution
# if __name__ == "__main__":
#     print(f"{Colors.BOLD}{Colors.CYAN}Starting 5-Minute Technical Analyzer with Robust Signal Confirmation...{Colors.END}")
    
#     # Create and start analyzer with 2-candle confirmation
#     analyzer = QuickScalpAnalyzer('TAOUSDT', confirmation_candles=2)
